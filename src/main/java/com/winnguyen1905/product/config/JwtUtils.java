...
public class JwtUtils {
  ...
  public JwtClaimsSet createJwtClaimsSet(MyUserDetails myUserDetails, Instant now, Instant validity) {
    List<Permission> permissions = myUserDetails.getRole() instanceof Role ? myUserDetails.getRole().getPermissions()
        : new ArrayList<>();
    myUserDetails.setRole(null);
    return JwtClaimsSet.builder().issuedAt(now).expiresAt(validity)
        .subject(myUserDetails.getUsername() + "/" + myUserDetails.getId()).claim("user", myUserDetails)
  ...
  public Pair<String, String> createTokenPair(MyUserDetails myUserDetails) {
    ...
    String accessToken = jwtEncoder
        .encode(JwtEncoderParameters.from(jwsHeader, createJwtClaimsSet(myUserDetails, now, accessTokenValidity)))
        .getTokenValue();
    String refreshToken = jwtEncoder
        .encode(JwtEncoderParameters.from(jwsHeader, createJwtClaimsSet(myUserDetails, now, refreshTokenValidity)))
        .getTokenValue();
